...
transformed data {
  ...
  for (i in 1:current_T) {
    stock_index_full[i] = stock_index[i];
  }
  for (i in (current_T + 1):T) {
    stock_index_full[i] = stock_index[current_T];
  }
}
parameters {
  vector[S] raw_mu_b_T;
...
transformed parameters {
  ...
  for (i in 1:T) {
    mu_b[:,i] = mu_b[:,i] + beta * rep_vector(stock_index_full[i], S);
  }
...
generated quantities {
  ...
  for (n in 1:N_state_polls_test) {
    ...
    logit_pi_tmp = mu_b[state_test[n], day_state_test[n]] +
      ...
      unadjusted_state_test[n] * e_bias[day_state_test[n]] +
  ...
  for (n in 1:N_national_polls_test) {
    ...
    logit_pi_tmp = national_mu_b_average[day_national_test[n]] +
      ...
      unadjusted_national_test[n] * e_bias[day_national[n]] +
