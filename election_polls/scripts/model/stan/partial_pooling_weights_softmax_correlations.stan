data {
  ...
  corr_matrix[d_discrete] state_correlations;
...
transformed parameters{
  ...
  matrix[N_test,K] f2;
  for(k in 1:(K-1)) {
    matrix[d_discrete, d_discrete] Sigma;
    matrix[d_discrete, d_discrete] L;
    Sigma = diag_matrix(rep_vector(sigma[k], d_discrete)) * state_correlations * diag_matrix(rep_vector(sigma[k], d_discrete));
    L = cholesky_decompose(Sigma);
    beta[k]= append_row(mu[k] + L * tau[k], beta_con[k]);
  }
  for(k in 1:(K-1)) {
    f[,k]= X * beta[k];
    f2[,k]= X_test * beta[k];
